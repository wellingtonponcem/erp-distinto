...
export function render(ctx: SlideCtx): string {
  ...
  const vencida = proposta.validade < hojeStr;
  const partesValidade = String(proposta.validade ?? '').split('-');
  const validadeFormatada = `${partesValidade[2]}/${partesValidade[1]}/${partesValidade[0]}`;

  return `<div class="theme-filmmaker">
    ...
    <section class="proposal-page" style="background: linear-gradient(rgba(0,0,0,0.6), rgba(0,0,0,0.8)), url('https://images.unsplash.com/photo-1492691523567-6170f0295dbd?auto=format&fit=crop&q=80&w=1200'); background-size: cover; background-position: center;">
        ...
        <div style="padding: 40px 10vw; display: flex; justify-content: space-between; align-items: center; border-top: 1px solid rgba(255,255,255,0.1);">
            ...
            <div style="font-size: 12px; opacity: 0.5; letter-spacing: 2px; text-transform: uppercase;">Production No. ${new Date().getFullYear()}-${Math.floor(Math.random() * 900) + 100}</div>
        </div>
    </section>
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 80px; align-items: center;">
                <div>
                    ...
                    <div style="font-size: 18px; line-height: 1.8; opacity: 0.8;">
                        ${d.secoes?.visao ?? 'Nossa missão é transformar seu evento em uma obra cinematográfica, focando na emoção e na estética de cada frame.'}
                    </div>
                </div>
    ...
    <section class="proposal-page">
        <div class="page-content" style="align-items: center; text-align: center;">
            ...
            <div style="border: 1px solid var(--accent); padding: 60px 100px; display: inline-block;">
                ...
                <div style="margin-top: 20px; font-size: 14px; opacity: 0.4;">Production code: #${mbUpper(slug.slice(0, 8))}</div>
            </div>

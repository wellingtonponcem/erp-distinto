...
- **Correção no Endpoint de Sincronização do Assinafy, Integração CRM e Clientes** *(jun/2026)*:
  - Causa raiz: Erro 404 (Página não encontrada) ao sincronizar manualmente, pois a rota GET para obter os detalhes do documento incluía erroneamente `/accounts/{accountId}` no início da URL. Além disso, o documento concluído no Assinafy retorna com o status `'certificated'`, que não era mapeado como `'assinado'` pelo ERP, impedindo a atualização automática e manual. Também havia a necessidade de atualizar o CRM (mover a oportunidade vinculada para `'ganha'`) e promover o lead inserindo seu CPF/CNPJ de assinatura e e-mail de contato na base de clientes.
  - Solução: Ajustado o endpoint no script `api/contratos/sincronizar_status.php` de `/accounts/{accountId}/documents/{documentId}` para `/documents/{documentId}`. Adicionado suporte aos status `'certificated'` e `'registrado'` mapeando-os para `'assinado'` tanto na sincronização manual quanto no `api/contratos/webhook_assinafy.php`. Implementada a atualização automática no CRM da oportunidade vinculada para a etapa `'ganha'` e a injeção do CPF/CNPJ e contato do signatário contratante do contrato assinado na base de `clientes` local em ambos os endpoints de atualização. Ajustados os links e botões no painel para exibir a opção "Visualizar / Baixar Assinado" quando o status for `'assinado'`.
